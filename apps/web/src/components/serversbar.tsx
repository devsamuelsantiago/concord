...
export default function ServersBar({ servers }: { servers: Server[] }) {
  return (
    <aside className="flex flex-col items-center h-screen w-[72px] bg-hue5 p-3 space-y-2">
      <div className="flex justify-center items-center rounded-full bg-hue2 hover:bg-hue1 w-12 h-12 hover:cursor-pointer active:scale-110 transition-all duration-200">
      ...
      <div className="flex flex-col space-y-2">
        {servers.map((server) => (
          <div key={server.id}>
            <ServerCircle
              bgColor="bg-hue3"
              notification={server.hasNotification}
            >
